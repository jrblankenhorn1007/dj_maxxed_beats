...
The initial inventory is based on integrated `origin/main`
...
branch. Add each future branch dossier on its working branch and update this
index when the branch is integrated.
...
The entries below cover the merged PR branches reported on the baseline
`origin/main` and terminal attempts that already have project records. New
work must create its dossier before opening a PR. Open or otherwise
unintegrated legacy branches are not represented as merged history; their
owners must follow the current prompt before merging and add their dossier
when integrated.
