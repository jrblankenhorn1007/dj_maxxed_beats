...
| Development process | Set up | Canonical TDD/Ralph instructions and the `ralph-loop` agent live in the shared `copilot_skills` repository; local skill/prompt files are redirect pointers. Before each agent run, the runner fetches `origin` and verifies the clean `main` integration worktree matches `origin/main`, then passes that evidence to the agent. It selects the shared agent, pins `gpt-6-luna`, creates one branch/worktree per iteration, opens a PR, requests a merge commit with `gh pr merge --merge`, retries while requirements are pending, verifies the reported merge commit on `origin/main`, then removes the successful local worktree/branch. The mocked test covers stale-ref refresh, preflight handoff, merge-commit and squash-style results, a retried pending-requirements response, and a closed-PR blocker, verifying agent/model selection, branch isolation, remote verification, marker ordering, blocker-commit persistence, and cleanup. |
...
- **Ralph branch/merge workflow:** `bash tests/ralph-status-reporting.sh`
  ...
  creation, refreshing a stale `origin/main` ref before agent launch, runner-
  owned clean-main preflight evidence, a merge-commit request, a retried
  pending-requirements response, merge-commit and squash-style verification on
  `origin/main`, status commits, shared-agent/model selection, marker ordering,
  cleanup, and a third iteration whose closed PR is recorded as a pushed
  `BLOCKED` status without a success marker.
...
- **Per-iteration branch/worktree workflow:** `bash
  ...
  `gpt-6-luna`, fresh branches/worktrees from updated `main`, refreshing stale
  `origin/main`, passing runner-owned main-worktree evidence to the agent, PR
  merges through the mocked GitHub CLI, retrying a pending merge request,
  remote merge-SHA verification for merge-commit and squash-style results,
